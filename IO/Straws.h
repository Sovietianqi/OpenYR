...
//========================================================================
// Straws - Data streaming / decompression pipeline
//
// Straws represent a chain of data sources that can be combined
// to perform multi-stage decompression and decryption.
//
// The pattern is:
//   FileStraw -> PKStraw -> BlowStraw -> (read data)
//
// Each Straw reads from its predecessor (ChainTo), transforms the
// data, and provides it to the next straw in the chain (ChainFrom).
//
// Flow direction: ChainFrom <-- Straw <-- ChainTo
//   Data flows from ChainTo (source) to ChainFrom (consumer)
//========================================================================

//========================================================================
// Straw - Abstract base class for data streams
//
// The base straw provides chaining infrastructure. Each straw
// can be linked to a source (ChainTo) and a consumer (ChainFrom).
// The Get() method retrieves data from the chain.
//========================================================================
...
public:
    ...
    virtual int32 Get(void* pBuffer, int32 length)
    {
        if (ChainTo)
            return ChainTo->Get(pBuffer, length);
...
private:
    DISABLE_COPY_AND_MOVE(Straw)
};

//========================================================================
// BufferStraw - Reads from a fixed memory buffer
//
// Provides data from a pre-allocated memory buffer. Used for
// reading from MIX file entries that have been loaded into memory.
//========================================================================
...
public:
    BufferStraw() = delete;
    BufferStraw(void* pBuffer, size_t nLength)
        : Straw()
        , Index(0)
    {
    ...
    virtual ~BufferStraw() noexcept override {}

    virtual int32 Get(void* pBuffer, int32 length) override final
    {
...
//========================================================================
// FileStraw - Reads from a file
//
// Connects to a RawFileClass or derived class to read data from
// a file on disk. This is the starting point of a straw chain
// that reads from a physical file.
//========================================================================
...
public:
    FileStraw() = delete;
    explicit FileStraw(CCFileClass* pFile)
        : Straw()
        , File(pFile)
        , HasOpened(false)
    {
    ...
    virtual ~FileStraw() noexcept override
    {
        if (HasOpened)
        {
            File->Close();
    ...
    {
        if (!File || !pBuffer || length <= 0) return 0;

        if (!HasOpened)
        {
            if (!File->Open(static_cast<int32>(FileAccessMode::Read)))
                return 0;
            HasOpened = true;
        }

        return File->Read(pBuffer, length);
    }
...
private:
    DISABLE_COPY_AND_MOVE(FileStraw)
};

//========================================================================
// PKStraw - PKWare compression decompressor
//
// Decompresses data that was compressed using the PKWare DCL
// (Data Compression Library) format. This is the compression
// used by Westwood for their MIX archives.
//
// The PKWare format uses a combination of LZSS-style compression
// with a sliding window dictionary. The format is:
// - Flag byte: 0x00 = raw copy, 0x01 = compressed block
// - Compressed blocks contain offset/length pairs
//========================================================================
...
public:
    explicit PKStraw()
        ...
        , BlockLength(0)
    {
        Buffer = static_cast<uint8*>(YRMemory::Allocate(PK_MAX_BLOCK_SIZE));
        if (Buffer)
        {
            memset(Buffer, 0, PK_MAX_BLOCK_SIZE);
        }
    }

    virtual ~PKStraw() noexcept override
    {
        if (Buffer)
        {
    ...
    // PKWare constants
    static constexpr int32 PK_MAX_BLOCK_SIZE = 8192;
    static constexpr int32 PK_WINDOW_SIZE = 4096;
    static constexpr int32 PK_THRESHOLD = 2;
...
private:
    DISABLE_COPY_AND_MOVE(PKStraw)
};

//========================================================================
// BlowStraw - Blowfish decryption straw
//
// Decrypts data that was encrypted using the Blowfish cipher.
// Westwood used Blowfish encryption for their MIX archives
...
public:
    BlowStraw() = delete;
    explicit BlowStraw(const void* pKey, int32 keyLength)
        : Straw()
        , KeyIndex(0)
    {
        // Store the key
    ...
    virtual ~BlowStraw() noexcept override {}

    virtual int32 Get(void* pBuffer, int32 length) override;

    // Blowfish constants
    static constexpr int32 BLOWFISH_MAX_KEY_SIZE = 56;
    static constexpr int32 BLOWFISH_BLOCK_SIZE = 8; // 64-bit blocks
...
private:
    DISABLE_COPY_AND_MOVE(BlowStraw)

    void InitializeBlowfish();
    void EncryptBlock(uint32& left, uint32& right);
    void DecryptBlock(uint32& left, uint32& right);
    uint32 F(uint32 x) const;
};

//========================================================================
// LCWStraw - LCW (LZ77) decompression straw
//
// Decompresses data that was compressed using the LCW (LZ77 variant)
// algorithm. This was used by Westwood for some file formats in
// Command & Conquer.
//========================================================================
...
public:
    LCWStraw() = delete;
    explicit LCWStraw(bool bControl, size_t nBlockSize)
        : Straw()
    ...
    virtual ~LCWStraw() noexcept override
    {
        if (Buffer)
        {
            YRMemory::Deallocate(Buffer);
...
inline int32 LCWStraw::Get(void* pBuffer, int32 length)
{
    if (!pBuffer || length <= 0) return 0;
    ...
    // Process any remaining buffer data first
    if (Counter > 0)
    {
        int32 copyCount = (Counter < remaining) ? Counter : remaining;
        memcpy(dest, Buffer, static_cast<size_t>(copyCount));
        if (copyCount < Counter)
        {
            memmove(Buffer, Buffer + copyCount, static_cast<size_t>(Counter - copyCount));
        }
        Counter -= copyCount;
        remaining -= copyCount;
        totalWritten += copyCount;
        dest += copyCount;
    }
    ...
    while (remaining > 0)
    {
        ...
        uint8 cmd;
        if (ChainTo->Get(&cmd, 1) != 1) break;

        if (cmd < 0x80)
        {
            // 0x00-0x7F: Short literal copy
            int32 count = static_cast<int32>(cmd) + 1;
            if (count > remaining) count = remaining;

            int32 bytesRead = ChainTo->Get(dest, count);
        ...
        {
            // 0x80-0xBF: Far copy from 2-byte offset
            int32 count = static_cast<int32>(cmd - 0x80) + 3;
            if (count > remaining) count = remaining;

            uint8 offsetBytes[2];
            if (ChainTo->Get(offsetBytes, 2) != 2) break;

            int32 offset = static_cast<int32>(offsetBytes[0]) |
                          (static_cast<int32>(offsetBytes[1]) << 8);
            const uint8* src = dest - offset;
        ...
        {
            // 0xC0-0xDF: Near copy from 1-byte offset
            int32 count = static_cast<int32>(cmd - 0xC0) + 3;
            if (count > remaining) count = remaining;

            uint8 offsetByte;
            if (ChainTo->Get(&offsetByte, 1) != 1) break;

            int32 offset = static_cast<int32>(offsetByte);
            const uint8* src = dest - offset;
        ...
        {
            // 0xE0-0xFF: Fill/RLE
            int32 count = static_cast<int32>(cmd - 0xE0) + 3;
            if (count > remaining) count = remaining;

            uint8 fillByte;
            if (ChainTo->Get(&fillByte, 1) != 1) break;

            memset(dest, fillByte, static_cast<size_t>(count));
...
//========================================================================
// PKStraw - PKWare Decompression Implementation
//========================================================================

// PKWare DCL format:
// The compressed stream consists of blocks. Each block starts with
// a 2-byte header:
//   uint16 typeAndLength
//   - Bit 15 (0x8000): Block type (0 = literal, 1 = compressed)
//   - Bits 0-14: Block length (excluding the 2-byte header)
//
// Literal blocks: just copy the bytes directly
// Compressed blocks: use LZSS-style decompression with a 4KB window

inline int32 PKStraw::Get(void* pBuffer, int32 length)
{
    if (!pBuffer || length <= 0) return 0;
    ...
    // Handle carry-over from previous call
    if (CarryOver > 0)
    {
        int32 copyCount = (CarryOver < remaining) ? CarryOver : remaining;
        memcpy(dest, Buffer, static_cast<size_t>(copyCount));

        if (copyCount < CarryOver)
        {
            memmove(Buffer, Buffer + copyCount, static_cast<size_t>(CarryOver - copyCount));
        }
        CarryOver -= copyCount;
        remaining -= copyCount;
        totalWritten += copyCount;
    ...
    while (remaining > 0)
    {
        ...
        {
            uint16 header;
            if (!ChainTo || ChainTo->Get(&header, 2) != 2)
                break;

            BlockType = (header >> 15) & 1;
            BlockLength = header & 0x7FFF;
        ...
        if (BlockType == 0)
        {
            // Literal block: copy directly from source
            int32 copyCount = (BlockSize < remaining) ? BlockSize : remaining;
            int32 bytesRead = ChainTo->Get(dest, copyCount);
            if (bytesRead <= 0) break;
        ...
        {
            // Compressed block: LZSS decompression
            uint8 flagByte;
            if (!ChainTo || ChainTo->Get(&flagByte, 1) != 1)
                break;

            BlockSize--;

            for (int32 bit = 0; bit < 8 && BlockSize > 0 && remaining > 0; ++bit)
            {
                if (flagByte & (1 << bit))
                {
                    // Compressed: read offset/length pair
                    uint8 pair[2];
                    if (!ChainTo || ChainTo->Get(pair, 2) != 2)
                    {
                        remaining = 0;
                        break;
                    }
                    ...
                    int32 offset = static_cast<int32>(pair[0]) |
                                  ((static_cast<int32>(pair[1]) & 0x0F) << 8);
                    int32 count = ((static_cast<int32>(pair[1]) >> 4) & 0x0F) + PK_THRESHOLD + 1;

                    if (count > remaining)
                        count = remaining;

                    // Copy from the sliding window
                    int32 windowPos = totalWritten - offset - 1;
                    if (windowPos < 0)
                    {
                        // Offset into the window buffer
                        // In a full implementation, we'd maintain a 4KB window
                        break;
                    }

                    uint8* src = static_cast<uint8*>(pBuffer) + windowPos;
                    for (int32 i = 0; i < count; ++i)
                    {
                        dest[i] = src[i];
                    }
                ...
                else
                {
                    ...
                    uint8 byte;
                    if (!ChainTo || ChainTo->Get(&byte, 1) != 1)
                    {
                        remaining = 0;
                        break;
                    }
...
//========================================================================
// BlowStraw - Blowfish Decryption Implementation
//========================================================================

// Blowfish is a 64-bit block cipher with a variable-length key.
// The algorithm uses a Feistel network with 16 rounds.
// Westwood used a simplified version with a fixed key.

// Blowfish P-array initialization values (hex digits of Pi)
static const uint32 BlowfishPInit[18] = {
...
// Blowfish S-box initialization values (hex digits of Pi, continued)
static const uint32 BlowfishSInit[4][256] = {
    {
...
void BlowStraw::InitializeBlowfish()
{
    ...
    memcpy(S, BlowfishSInit, sizeof(S));
    ...
    // Encrypt the zero block with the key-modified P-array
    ...
    // Encrypt the zero block for each S-box entry
    for (int32 i = 0; i < 4; ++i)
    {
        for (int32 k = 0; k < 256; k += 2)
        {
            EncryptBlock(left, right);
            S[i][k] = left;
            S[i][k + 1] = right;
...
uint32 BlowStraw::F(uint32 x) const
{
    ...
    uint16 c = static_cast<uint16>((x >> 8) & 0xFF);
    uint16 d = static_cast<uint16>(x & 0xFF);
...
inline int32 BlowStraw::Get(void* pBuffer, int32 length)
{
    if (!pBuffer || length <= 0) return 0;
    ...
    // Read raw encrypted data from the chain
    int32 bytesRead = ChainTo->Get(pBuffer, length);
    if (bytesRead <= 0) return 0;

    // Decrypt in 8-byte blocks
    uint8* data = static_cast<uint8*>(pBuffer);
    int32 blocks = bytesRead / BLOWFISH_BLOCK_SIZE;

    for (int32 i = 0; i < blocks; ++i)
    {
        uint8* block = data + (i * BLOWFISH_BLOCK_SIZE);

        // Read as two 32-bit words (big-endian)
        uint32 left = (static_cast<uint32>(block[0]) << 24) |
                     (static_cast<uint32>(block[1]) << 16) |
                     (static_cast<uint32>(block[2]) << 8) |
                     static_cast<uint32>(block[3]);
        uint32 right = (static_cast<uint32>(block[4]) << 24) |
        ...
        block[0] = static_cast<uint8>(left >> 24);
        block[1] = static_cast<uint8>(left >> 16);
        block[2] = static_cast<uint8>(left >> 8);
        block[3] = static_cast<uint8>(left);
        block[4] = static_cast<uint8>(right >> 24);
        block[5] = static_cast<uint8>(right >> 16);
        block[6] = static_cast<uint8>(right >> 8);
        block[7] = static_cast<uint8>(right);
    }
