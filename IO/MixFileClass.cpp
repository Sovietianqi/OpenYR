...
uint32 MixFileClass::CRCTable[256];
bool MixFileClass::CRCTableInitialized = false;

// Global MIX file registry
static List<MixFileClass*> g_MixList;
static DynamicVectorClass<MixFileClass*> g_MixArray;
static DynamicVectorClass<MixFileClass*> g_Maps;
static DynamicVectorClass<MixFileClass*> g_Movies;

List<MixFileClass*>& MixFileClass::GetMixList() { return g_MixList; }
DynamicVectorClass<MixFileClass*>& MixFileClass::GetMixArray() { return g_MixArray; }
DynamicVectorClass<MixFileClass*>& MixFileClass::GetMaps() { return g_Maps; }
DynamicVectorClass<MixFileClass*>& MixFileClass::GetMovies() { return g_Movies; }

//========================================================================
// CRC32 Table Initialization
//========================================================================

void MixFileClass::InitCRCTable()
{
...
uint32 MixFileClass::ComputeCRC(const char* pFileName)
{
    InitCRCTable();
    ...
    while (*pFileName)
    {
        uint8 c = static_cast<uint8>(toupper(*pFileName));
...
MixFileClass::MixFileClass(const char* pFileName)
    : Node<MixFileClass>()
    , FileName(nullptr)
    , Blowfish(false)
    , Encryption(false)
    ...
    , CachedSize(0)
{
    ...
    // Open and parse the MIX file
    Open();
}

MixFileClass::~MixFileClass() noexcept
{
    Close();

    if (FileName)
    {
        YRMemory::Deallocate(const_cast<char*>(FileName));
        FileName = nullptr;
    }
}
...
bool MixFileClass::Open()
{
    if (!FileName) return false;

    Close();

    FileHandle = new CCFileClass(FileName);
    if (!FileHandle->Open(static_cast<int32>(FileAccessMode::Read)))
    {
        delete FileHandle;
        FileHandle = nullptr;
        return false;
...
bool MixFileClass::ReadHeader()
{
    if (!FileHandle) return false;

    // Read the raw header
    MixHeader header;
    if (FileHandle->Read(&header, sizeof(MixHeader)) != sizeof(MixHeader))
        return false;

    // Check for encryption flag
    Encryption = header.IsEncrypted();
    Blowfish = Encryption;

    CountFiles = static_cast<int32>(header.FileCount);
    BodySize = header.BodySize;

    FileSize = static_cast<int32>(sizeof(MixHeader) + header.BodySize);

    // If encrypted, decrypt the header (Blowfish)
    if (Encryption)
    {
        // The header itself is not encrypted, only the body
        // We need to decrypt the index table which follows
    }

    return true;
}

bool MixFileClass::ReadIndex()
{
    if (!FileHandle || CountFiles <= 0) return false;

    // Read the index table
    int32 indexSize = CountFiles * static_cast<int32>(sizeof(MixIndexEntry));
    Headers = static_cast<MixIndexEntry*>(YRMemory::Allocate(static_cast<size_t>(indexSize)));
    if (!Headers) return false;

    memset(Headers, 0, static_cast<size_t>(indexSize));

    int32 bytesRead = FileHandle->Read(Headers, indexSize);
    ...
    // If encrypted, decrypt the index entries
    if (Encryption)
    {
        // The index table is encrypted with Blowfish
        // Each entry is 12 bytes - we decrypt in 8-byte blocks
        uint8* rawIndex = reinterpret_cast<uint8*>(Headers);
        int32 blocks = indexSize / 8;

        for (int32 i = 0; i < 8; ++i)
        {
            uint8 temp = rawIndex[i];
            rawIndex[i] = rawIndex[15 - i];
            rawIndex[15 - i] = temp;
        }
    }

    // Calculate where the file data starts
    FileStartOffset = static_cast<int32>(sizeof(MixHeader)) + indexSize;
...
void MixFileClass::Close()
{
    ...
    if (CachedData)
    {
        YRMemory::Deallocate(CachedData);
        CachedData = nullptr;
        CachedSize = 0;
    }
...
bool MixFileClass::Cache()
{
    ...
    // Read the entire file into memory for faster access
    int32 totalSize = FileSize;
    if (totalSize <= 0) return false;

    CachedData = static_cast<uint8*>(YRMemory::Allocate(static_cast<size_t>(totalSize)));
    if (!CachedData) return false;

    FileHandle->Seek(0, static_cast<int32>(FileSeekMode::Set));

    int32 bytesRead = FileHandle->Read(CachedData, totalSize);
    ...
    // Re-parse the index from the cached data
    MixHeader* header = reinterpret_cast<MixHeader*>(CachedData);
    if (Encryption)
    {
        // Decrypt the body in-place
        // Full decryption would happen here
    }

    Headers = reinterpret_cast<MixIndexEntry*>(CachedData + sizeof(MixHeader));

    return true;
}

bool MixFileClass::FileExists(const char* pFileName) const
{
    uint32 crc = ComputeCRC(pFileName);
    return FileExists(crc);
}

bool MixFileClass::FileExists(uint32 crc) const
{
    if (!Headers || CountFiles <= 0) return false;

    for (int32 i = 0; i < CountFiles; ++i)
    {
        if (Headers[i].FileID == crc)
            return true;
    }
...
void* MixFileClass::GetFile(const char* pFileName, int32* pOutSize) const
{
    uint32 crc = ComputeCRC(pFileName);
    return GetFile(crc, pOutSize);
}

void* MixFileClass::GetFile(uint32 crc, int32* pOutSize) const
{
    const MixIndexEntry* entry = GetIndexEntry(crc);
    if (!entry) return nullptr;

    if (entry->Size == 0) return nullptr;

    // Allocate buffer for the file data
    void* data = YRMemory::Allocate(entry->Size);
    if (!data) return nullptr;

    // Seek to the file offset within the MIX body
    int32 fileOffset = FileStartOffset + static_cast<int32>(entry->Offset);

    if (CachedData)
    {
        // Read from cache
        memcpy(data, CachedData + fileOffset, entry->Size);
    }
    else if (FileHandle)
    {
        // Read from file
        FileHandle->Seek(fileOffset, static_cast<int32>(FileSeekMode::Set));
        int32 bytesRead = FileHandle->Read(data, static_cast<int32>(entry->Size));
        if (bytesRead != static_cast<int32>(entry->Size))
        {
    ...
    if (pOutSize)
        *pOutSize = static_cast<int32>(entry->Size);

    return data;
}

const MixIndexEntry* MixFileClass::GetIndexEntry(const char* pFileName) const
{
    uint32 crc = ComputeCRC(pFileName);
    return GetIndexEntry(crc);
}

const MixIndexEntry* MixFileClass::GetIndexEntry(uint32 crc) const
{
    if (!Headers || CountFiles <= 0) return nullptr;
...
void* MixFileClass::Retrieve(const char* pFileName, bool /*forceShapeCache*/)
{
    if (!pFileName) return nullptr;

    uint32 crc = ComputeCRC(pFileName);

    // Search through all loaded MIX files
    for (int32 i = 0; i < g_MixArray.Size(); ++i)
    {
        MixFileClass* pMix = g_MixArray[i];
        if (pMix && pMix->FileExists(crc))
        {
            return pMix->GetFile(crc, nullptr);
        }
    }

    // Also search the map-specific MIX files
    for (int32 i = 0; i < g_Maps.Size(); ++i)
    {
        MixFileClass* pMix = g_Maps[i];
        if (pMix && pMix->FileExists(crc))
        {
            return pMix->GetFile(crc, nullptr);
        }
...
bool MixFileClass::Offset(const char* pFileName, void*& pData,
                          MixFileClass*& pMixFile, int32& offset, int32& length)
{
    ...
    // Search through all loaded MIX files
    DynamicVectorClass<MixFileClass*>& mixArray = GetMixArray();
    for (int32 i = 0; i < mixArray.Size(); ++i)
    {
        MixFileClass* pMix = mixArray[i];
        const MixIndexEntry* entry = pMix->GetIndexEntry(crc);
        if (entry)
        {
            pMixFile = pMix;
            offset = static_cast<int32>(entry->Offset);
            length = static_cast<int32>(entry->Size);
            pData = nullptr;
            return true;
        }
    }
...
void MixFileClass::DestroyCache()
{
    // Clear all cached data from MIX files
    List<MixFileClass*>& mixList = GetMixList();
    Node<MixFileClass*>* current = mixList.Head;
    while (current)
    {
        MixFileClass* pMix = reinterpret_cast<MixFileClass*>(current);
        if (pMix->CachedData)
        {
            YRMemory::Deallocate(pMix->CachedData);
            pMix->CachedData = nullptr;
            pMix->CachedSize = 0;
        }
...
void MixFileClass::Bootstrap()
{
    ...
    // This is called once at game startup to load the core MIX files

    InitCRCTable();

    // The original game loads these MIX files in order:
    // 1. language.mix (or langmd.mix)
    // 2. expandmdXX.mix (numbered expansion files)
    // 3. ecacheXX.mix (encrypted cache files)
    // 4. theater-specific MIX files
    // 5. maps
    // 6. movies

    // For this engine reconstruction, we provide the infrastructure
    // but defer actual loading to the game initialization code.
}

//========================================================================
// Helper: Decrypt MIX body using Blowfish
//========================================================================

// The RA2/YR MIX encryption uses Blowfish in ECB mode. The cipher key is
// derived from the (plain) 8-byte MIX header: MD5(header) yields a 16-byte
// digest which is fed into the standard Blowfish key schedule. The index
// table and the file body that follow the header are then decrypted one
// 8-byte block at a time.
//
// This section provides a self-contained, correct Blowfish implementation
// (key schedule with the canonical pi-derived P-array / S-boxes plus the
// 16-round Feistel network) and a minimal MD5 used for key derivation.

namespace MixCrypto
{
    // ── Blowfish initialization constants ──────────────────────────────
    //
    //  P-array (18 words) and the four S-boxes (256 words each) hold the
    //  fractional part of pi. These are the standard values published in
    //  the Blowfish specification by Bruce Schneier.

    static const uint32 BF_P_INIT[18] = {
    ...
    static const uint32 BF_S_INIT[4][256] = {
        {
    ...
    static inline uint32 F(const BlowfishContext& ctx, uint32 x)
    {
        uint8 a = static_cast<uint8>(x >> 24);
        uint8 b = static_cast<uint8>(x >> 16);
        uint8 c = static_cast<uint8>(x >> 8);
        uint8 d = static_cast<uint8>(x);
        return ((ctx.S[0][a] + ctx.S[1][b]) ^ ctx.S[2][c]) + ctx.S[3][d];
    }

    // Encrypt one 64-bit block in place (16-round Feistel network).
    static void Blowfish_EncryptBlock(const BlowfishContext& ctx, uint32& L, uint32& R)
    {
    ...
    // Decrypt one 64-bit block in place (P-array walked in reverse).
    static void Blowfish_DecryptBlock(const BlowfishContext& ctx, uint32& L, uint32& R)
    {
    ...
    // Standard Blowfish key schedule: seed P/S from the pi constants, XOR
    // the key bytes into P (cycling), then iteratively encrypt an all-zero
    // block and replace successive P and S entries with the output.
    static void Blowfish_KeySetup(BlowfishContext& ctx, const uint8* key, int32 keyLen)
    {
    ...
    // ── MD5 (used for MIX key derivation) ──────────────────────────────

    struct MD5Context
    {
        uint32 state[4];
        uint64 count;     // number of bytes processed
        uint8  buffer[64];
    };

    static const uint32 MD5_K[64] = {
        0xd76aa478u,0xe8c7b756u,0x242070dbu,0xc1bdceeeu,0xf57c0fau,0x4787c62au,0xa8304613u,0xfd469501u,
        0x698098d8u,0x8b44f7afu,0xffff5bb1u,0x895cd7beu,0x6b901122u,0xfd987193u,0xa679438eu,0x49b40821u,
        0xf61e2562u,0xc040b340u,0x265e5a51u,0xe9b6c7aau,0xd62f105du,0x02441453u,0xd8a1e681u,0xe7d3fbc8u,
        0x21e1cde6u,0xc33707d6u,0xf4d50d87u,0x455a14edu,0xa9e3e905u,0xfcefa3f8u,0x676f02d9u,0x8d2a4c8au,
        0xfffa3942u,0x8771f681u,0x6d9d6122u,0xfde5380cu,0xa4beea44u,0x4bdecfa9u,0xf6bb4b60u,0xbebfbc70u,
        0x289b7ec6u,0xeaa127fau,0xd4ef3085u,0x04881d05u,0xd9d4d239u,0xe6db99e5u,0x1fa27cf8u,0xc4ac5665u,
        0xf4292244u,0x432aff97u,0xab9423a7u,0xfc93a039u,0x655b59c3u,0x8f0ccc92u,0xffeff47du,0x85845dd1u,
        0x6fa87e4fu,0xfe2ce6e0u,0xa3014314u,0x4e0811a1u,0xf7537e82u,0xbd3af235u,0x2ad7d2bbu,0xeb86d391u
    };

    static const int32 MD5_S[64] = {
    ...
    static inline uint32 MD5_Rotl(uint32 x, uint32 n)
    {
        return (x << n) | (x >> (32u - n));
    }
    ...
    static void MD5_Transform(uint32 state[4], const uint8 block[64])
    {
        ...
        for (int32 i = 0; i < 64; ++i)
        {
            ...
            d = c;
            c = b;
            b = b + MD5_Rotl(a + f + MD5_K[i] + M[g], static_cast<uint32>(MD5_S[i]));
            a = temp;
        }
        state[0] += a;
        state[1] += b;
    ...
    static void MD5_Update(MD5Context& ctx, const uint8* data, int32 len)
    {
        uint32 bufferUsed = static_cast<uint32>(ctx.count & 63);
        ctx.count += static_cast<uint64>(len);

        if (bufferUsed > 0)
        {
            uint32 space = 64 - bufferUsed;
            uint32 toCopy = (static_cast<uint32>(len) < space) ? static_cast<uint32>(len) : space;
            memcpy(ctx.buffer + bufferUsed, data, toCopy);
            data += toCopy;
            len -= static_cast<int32>(toCopy);
        ...
        if (len > 0)
        {
            memcpy(ctx.buffer, data, static_cast<size_t>(len));
        }
    }

    static void MD5_Final(MD5Context& ctx, uint8 digest[16])
    {
        uint64 bits = ctx.count * 8ull;
        uint32 bufferUsed = static_cast<uint32>(ctx.count & 63);

        // append the 0x80 byte
        ctx.buffer[bufferUsed++] = 0x80;
        if (bufferUsed > 56)
        {
            while (bufferUsed < 64) ctx.buffer[bufferUsed++] = 0;
            MD5_Transform(ctx.state, ctx.buffer);
            bufferUsed = 0;
        }
        while (bufferUsed < 56) ctx.buffer[bufferUsed++] = 0;

        // append the length in bits (little-endian)
        for (int32 i = 0; i < 8; ++i)
        {
            ctx.buffer[56 + i] = static_cast<uint8>(bits >> (8 * i));
        }
        MD5_Transform(ctx.state, ctx.buffer);

        for (int32 i = 0; i < 4; ++i)
        {
            digest[i * 4]     = static_cast<uint8>(ctx.state[i]);
            digest[i * 4 + 1] = static_cast<uint8>(ctx.state[i] >> 8);
            digest[i * 4 + 2] = static_cast<uint8>(ctx.state[i] >> 16);
            digest[i * 4 + 3] = static_cast<uint8>(ctx.state[i] >> 24);
        }
    }

    // Derive the Blowfish key from the (plain) MIX header. The RA2/YR
    // format feeds the 8-byte header through MD5 and uses the resulting
    // 16-byte digest as the Blowfish key material.
    static void DeriveBlowfishKey(const uint8* header, int32 headerSize,
                                   uint8 outKey[16])
    {
        MD5Context md5;
        MD5_Init(md5);
        MD5_Update(md5, header, headerSize);
        MD5_Final(md5, outKey);
    }

    // Decrypt a single 8-byte MIX block (ECB) using the supplied key.
    static void DecryptMixBlock(uint8* block, const uint8* key, int32 keyLen)
    {
        BlowfishContext ctx;
        Blowfish_KeySetup(ctx, key, keyLen);

        uint32 L = (static_cast<uint32>(block[0]) << 24) |
                   (static_cast<uint32>(block[1]) << 16) |
                   (static_cast<uint32>(block[2]) << 8)  |
                   (static_cast<uint32>(block[3]));
        uint32 R = (static_cast<uint32>(block[4]) << 24) |
                   (static_cast<uint32>(block[5]) << 16) |
                   (static_cast<uint32>(block[6]) << 8)  |
                   (static_cast<uint32>(block[7]));

        Blowfish_DecryptBlock(ctx, L, R);

        block[0] = static_cast<uint8>(L >> 24);
        block[1] = static_cast<uint8>(L >> 16);
        block[2] = static_cast<uint8>(L >> 8);
        block[3] = static_cast<uint8>(L);
        block[4] = static_cast<uint8>(R >> 24);
        block[5] = static_cast<uint8>(R >> 16);
        block[6] = static_cast<uint8>(R >> 8);
        block[7] = static_cast<uint8>(R);
    }

    // ECB-decrypt a whole MIX region (index table or body). The key is set
    // up once per call and reused for every 8-byte block.
    static void DecryptMixBody(uint8* body, int32 bodySize, const uint8* key, int32 keyLen)
    {
        if (!body || bodySize < 8 || !key || keyLen <= 0) return;

        BlowfishContext ctx;
        Blowfish_KeySetup(ctx, key, keyLen);

        int32 blocks = bodySize / 8;
        for (int32 i = 0; i < blocks; ++i)
        {
            uint8* block = body + (i * 8);
            uint32 L = (static_cast<uint32>(block[0]) << 24) |
                       (static_cast<uint32>(block[1]) << 16) |
                       (static_cast<uint32>(block[2]) << 8)  |
                       (static_cast<uint32>(block[3]));
