...
本项目为开源学习与研究工作的一部分，旨在促进对经典游戏引擎的技术理解与社区创新。使用者需明确知晓以下条款：

- 非官方性质：OpenYR 并非 Electronic Arts 或其关联公司的官方产品，亦未经授权或认可。
- 逆向工程风险：项目基于对原版二进制文件的逆向分析，可能涉及法律灰色地带。使用本项目的源代码或编译产物所产生的任何法律后果，由使用者自行承担。
- 不保证兼容性：由于逆向分析存在固有误差，OpenYR 的实现可能与原版行为存在差异，也可能导致游戏资源不兼容、存档损坏或其他不可预见的问题。项目团队不对因使用本软件而产生的任何损失负责。
- 仅供研究目的：建议使用者仅将本项目用于技术学习、个人研究或非商业用途，严禁用于侵犯任何第三方权益的行为。
